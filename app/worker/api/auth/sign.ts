...
async function sign(path: string, expiry: number) {
  ...
  const receivedMac = byteStringToUint8Array(_atob(receivedMacBase64));
  ...
  // The request is authenticated, so return the response.
