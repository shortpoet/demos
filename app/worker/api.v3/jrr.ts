...
type RouterType = { routes: any[]; handle: (req: IncomingMessage, res: ServerResponse, ...args: any[]) => Promise<void> };
type RequestLike = IncomingMessage & { params?: { [key: string]: any }; query?: { [key: string]: any }; proxy?: any; body?: any };
...
const createRouter = ({ base = '', routes = [] }: RouterOptions = {}): RouterType => ({
  ...
  async handle(req: IncomingMessage, res: ServerResponse, ...args) {
    let response, match, url = new URL(req.url!, `http://${req.headers.host}`);
    ...
    for (let [method, route, handlers] of routes) {
      if ((method === req.method || method === 'ALL') && (match = url.pathname.match(route))) {
