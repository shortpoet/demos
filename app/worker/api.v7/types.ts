...
import { IncomingMessage as _IncomingMessage } from "unenv/runtime/node/http";
import { ServerResponse } from "unenv/runtime/node/http";
...
export type WorkerMiddleware<Body = any> = (
...
export interface WorkerAppUse {
  <Body>(
    route: string | string[],
    handle: WorkerMiddleware<Body> | WorkerMiddleware<Body>[],
    options?: Partial<WorkerInputLayer>
  ): WorkerApp;
  <Body>(
    route: string | string[],
...
export type WorkerHandle<T = any> = (
