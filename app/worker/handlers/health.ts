...
export const __healthCheck = async (c: any) => {
  const env = c.env;
  let gitInfo;
  let version = "";
  ...
  let corsify;
  console.log("env.API_VERSION", env.API_VERSION);
  // this might not be needed but only seems to work globally with v6, but before v5, v1?
  // switch (env.API_VERSION) {
  //   case "v3":
  //     corsify = (await import("../api.v3")).corsify;
  //     break;
  ...
  //     break;
  // }
  corsify = (await import("../api.v6")).corsify;
